...
public class DeviceController extends BaseController {
    ...
    @ResponseBody
    @Operation(summary = "获取设备上下文", description = "供硬件端调用：获取归属馆 ID 及当前绑定的展品 ID")
    public ResultMessage getDeviceContext(@PathVariable String deviceId) {
        try {
            SysDevice device = deviceService.selectDeviceById(deviceId);
            if (device == null) {
                return ResultMessage.error("设备未激活或不存在");
            }
            Map<String, Object> context = new HashMap<>();
            context.put("deviceId", deviceId);
            context.put("museumId", device.getMuseumId());
            context.put("exhibitId", device.getExhibitId());
            context.put("deviceName", device.getDeviceName());
            return ResultMessage.success(context);
        } catch (Exception e) {
            logger.error("获取设备上下文失败: " + deviceId, e);
            return ResultMessage.error("系统异常");
        }
    }
    ...
    public ResultMessage create(@Valid @RequestBody DeviceAddParam param) {
        try {
            ...
            device.setUserId(CmsUtils.getUserId());
            device.setDeviceName(query.getType()!= null && !query.getType().isEmpty() ? query.getType() : "智博导览终端");
            device.setType(query.getType());
            device.setDeviceId(query.getDeviceId());
            device.setMuseumId(param.getMuseumId());
            device.setExhibitId(param.getExhibitId());
