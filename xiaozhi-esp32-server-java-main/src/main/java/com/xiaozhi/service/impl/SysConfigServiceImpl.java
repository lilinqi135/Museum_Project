...
public class SysConfigServiceImpl extends BaseServiceImpl implements SysConfigService {
    ...
    public SysConfig selectModelType(String modelType) {
        // embedding 配置变更频率高且会直接影响 RAG 可用性，避免命中陈旧缓存
        if (SysConfig.ModelType.embedding.getValue().equals(modelType)) {
            return loadModelTypeFromDb(modelType);
        }
        ...
        return cacheHelper.getWithLock(
            ...
            // 从数据库获取
            () -> loadModelTypeFromDb(modelType)
        );
    }

    private SysConfig loadModelTypeFromDb(String modelType) {
        ...
        if (result != null && !SysConfig.ModelType.embedding.getValue().equals(modelType)) {
            Cache cache = cacheManager.getCache(CACHE_NAME);
            if (cache != null) {
